...
class ValueNetwork(nn.Module):
    ...
    def __call__(
        self, prepared_player_hands: jnp.ndarray, table_state: jnp.ndarray
    ) -> jnp.ndarray:
...
def call_value_network(
    ...
    return jnp.array(
        value_network.apply(value_network_params, prepared_player_hands, table_state)
    )
...
def compute_value_loss(
    ...
    predicted_values = call_value_network(
        value_network, params, prepared_player_hands, table_states
    )
...
def compute_policy_loss(
    ...
    logits = call_policy_network(
        policy_network, params, prepared_knowledge, table_states, encoded_actions
    )
...
compute_value_loss_vect_raw = vmap(
    compute_value_loss, in_axes=(None, None, 0, 0, 0), out_axes=0
)
compute_policy_loss_vect_raw = vmap(
    compute_policy_loss, in_axes=(None, None, 0, 0, 0, 0), out_axes=0
)
...
def compute_value_loss_vect(
    ...
    return compute_value_loss_vect_raw(
        value_network, params, prepared_player_hands, table_states, target_values
    ).mean()
